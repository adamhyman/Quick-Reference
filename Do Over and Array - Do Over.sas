%MACRO DO_OVER(arraypos, array=, 
 ...
 /*  Last modified: 8/4/2006
  ...
  Parameters:
     ...
     MONTHS = A numbered lists of the form 201201-201512, data_1201-data_1512, etc.
  ...
  Rules:
      Exactly one of ARRAYPOS or ARRAY or VALUES or MONTHS is required.
  ...
  History
    ...
    6/6/13  allow MONTHS= to do over a list of months.
*/
...
%else %if %quote(&months) ne %then %let prefixes=_Intrnl;
...
%* Load the MONTHS into macro array 1 (only one is permitted);
%if %length(%str(&MONTHS)) >0 %THEN %do;
    %* Check for numbered list of form xxx-xxx and expand it 
    using NUMLIST macro.;
    %IF (%INDEX(%STR(&MONTHS),-) GT 0) and 
        (%SCAN(%str(&MONTHS),2,-) NE ) and 
        (%SCAN(%str(&MONTHS),3,-) EQ ) 
        %THEN %LET MONTHS=%MONTHLIST(&MONTHS);

    %do iter=1 %TO 9999;  
        %let val=%scan(%str(&MONTHS),&iter,%str(&DELIM));
        %if %quote(&VAL) ne %then %do;
            %let &PREFIX1&ITER=&VAL;
            %let &PREFIX1.N=&ITER;
            %end;
