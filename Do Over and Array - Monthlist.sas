%MACRO MONTHLIST(listwithdash); 
  /* 
	Lines 95 and 96 added by Adam
...
%do _III_=&_from %to &_to;
		...
		%if %substr(&_XXX_,%eval(%length(&_iii_)-1),2) le 12 %then
		%if %substr(&_XXX_,%eval(%length(&_iii_)-1),2) ne 00 %then
		%TRIM(&stem&_XXX_)
		;
%end; 
