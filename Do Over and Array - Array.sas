...
%MACRO ARRAY(arraypos, array=, data=, var=, values=, months=,
                       delim=%STR( ), debug=N, numlist=Y);
/*
Modified for Directv by Adam Hyman - ahyman@directv.com
...
Function: Define one or more Macro Arrays
 ...
 Parameters: 
    ...
    MONTHS  = A numbered list, ending in YYYYMM format,
              eg 201201-201512, RIO_201201-RIO_201512.
 ...
 REQUIRED OTHER MACRO:  Requires NUMLIST if using numbered lists are used
              ...
              Requires MONTHLIST if using the MONTHS parameter.

 How the program works.
    When the VALUES or MONTHS parameters are used, it is parsed into individual 
 ...
 History
  ...
  6/6/13 allow MONTHS= to create an array of months.
...
%IF %LENGTH(%STR(&VALUES)) >0 %THEN %DO;
    %IF &NUMLIST=Y %then %DO;
        %* Check for numbered list of form xxx-xxx and expand it using the NUMLIST macro.;
        %IF (%INDEX(%quote(&VALUES),-) GT 0) and (%length(%SCAN(%quote(&VALUES),1,-))>0) and (%length(%SCAN(%quote(&VALUES),2,-))>0) and (%length(%SCAN(%quote(&VALUES),3,-))=0) 
    ...
    %DO J=1 %TO &PREFIXN;
        %DO ITER=1 %TO 9999;  
            ...
            %ELSE %goto out3;
            %END; 
        %out3: %IF &&&&&&PREFIX&J..N LT &MINLENG %THEN %LET MINLENG=&&&&&&PREFIX&J..N;
        %END;
    %if &PREFIXN >1 %THEN %DO J=1 %TO &PREFIXN;
        %IF &&&&&&PREFIX&J..N NE &MINLENG %THEN %PUT ERROR: Number of values must be a multiple of the number of arrays;
        %END;
    %END;


%*------------------------------------------------------;
%*  CASE 2: MONTHS parameter is used
%*------------------------------------------------------;

%ELSE %IF %LENGTH(%STR(&MONTHS)) >0 %THEN %DO;
    %IF &NUMLIST=Y %then %DO;
        %* Check for numbered list of form xxx-xxx and expand it using the NUMLIST macro.;
        %IF (%INDEX(%quote(&MONTHS),-) GT 0) and (%length(%SCAN(%quote(&MONTHS),1,-))>0) and (%length(%SCAN(%quote(&MONTHS),2,-))>0) and (%length(%SCAN(%quote(&MONTHS),3,-))=0) 
            %THEN %LET MONTHS=%MONTHLIST(&MONTHS);
        %END;
    %LET MINLENG=99999;
    %DO J=1 %TO &PREFIXN;
        %DO ITER=1 %TO 9999;  
            %LET WHICH=%EVAL((&ITER-1)*&PREFIXN +&J); 
            %LET VAL=%SCAN(%STR(&MONTHS),&WHICH,%STR(&DELIM));
            %IF %QUOTE(&VAL) NE %THEN %DO;
                %GLOBAL &&&&PREFIX&J..&ITER;
                %LET &&&&PREFIX&J..&ITER=&VAL;
                %LET &&&&PREFIX&J..N=&ITER;
            ...
            %ELSE %goto out4;
            %END; 
        %out4: %IF &&&&&&PREFIX&J..N LT &MINLENG %THEN %LET MINLENG=&&&&&&PREFIX&J..N;
        %END;
    %if &PREFIXN >1 %THEN %DO J=1 %TO &PREFIXN;
        %IF &&&&&&PREFIX&J..N NE &MINLENG %THEN %PUT ERROR: Number of values must be a multiple of the number of arrays;
        %END;
...
%*------------------------------------------------------;
%*  CASE 3: DATA and VAR parameters used
%*------------------------------------------------------;
